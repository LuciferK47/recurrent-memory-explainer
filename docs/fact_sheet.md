...
- Fraunces font: SIL Open Font License
- Departure Mono font (self-hosted, `react-app/public/fonts/`): SIL Open Font License 1.1
- JetBrains Mono: SIL Open Font License
- KaTeX: MIT License
- React / React DOM: MIT License
- Tailwind CSS, Vite, PostCSS, TypeScript (build tooling): MIT License
- lucide-react: ISC License
- motion (Framer Motion): MIT License
- NumPy (`scripts/precompute_interference.py`): BSD 3-Clause License
- `data/interference_sweep.json`: original, generated by `scripts/precompute_interference.py` — MIT License (same as the artifact code)
- `data/bdh_cq_demo_tasks.json`: **original ARC-style examples authored for this explainer — not sourced from the ARC-AGI dataset.** See the file's own `description` field and `scripts/precompute_bdh_demo.py`. MIT License (same as the artifact code)
- All original graphics (the isometric illustration suite, the Stage's canvas/SVG renderers, the background circuit-city): original work, MIT License (same as the artifact code)

Full third-party attribution record: `ATTRIBUTIONS.md` (repo root).
