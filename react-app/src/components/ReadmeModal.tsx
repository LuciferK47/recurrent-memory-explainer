...
const README_MARKDOWN = `# Recurrent Memory Explainer: How Fixed-Size State Learns and Forgets
...
│   │   │   ├── ReadmeModal.tsx    # In-app rendered markdown & KaTeX README viewer
...
│   ├── readme.html                # Pre-rendered standalone README viewer with KaTeX
...
export const ReadmeModal: React.FC<ReadmeModalProps> = ({ isOpen, onClose }) => {
  ...
  const renderedHtml = useMemo(() => {
    ...
    const processedMath = README_MARKDOWN.replace(/\$([^\$\n]+)\$/g, (_, expr) => {
      try {
    ...
    >
      <div className="bg-canvas border border-border rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-fadeIn">
        ...
        <div className="px-6 py-3 border-t border-border bg-surface flex items-center justify-between text-xs text-ink-muted">
          <span>Source: <code className="font-mono text-[11px] bg-linen px-1 py-0.5 rounded border border-border">README.md</code></span>
